#!/usr/bin/python
import sys,os,mpdclient2,xmpp,time,md5
	...
	while True:
		...
		try:
			...
			if status.has_key('time'):
				length = status.time
			artist = ''
			if mpd_song.has_key('artist'):
				artist = mpd_song['artist']
			source = ''
			if mpd_song.has_key('path'):
				source = mpd_song['path']
			if artist == '' and title.count(' - ') > 0:
				parts = title.split(' - ')
				artist = parts[0]
				title = parts[1]
			if length.__str__().count(':') > 0:
				parts = length.split(':');
				position = parts[0];
				length = parts[1];
			if last_song != title:
